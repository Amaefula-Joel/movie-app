        ...
        <div className="grow-span-1">
            <div className="rounded-2xl rounded-br-none bg-[#e0e0e0] dark:bg-[#323232]">
                <div className="relative group">
                    ...
                    <a href={`/details/${type}/${id}`} className="absolute inset-0 bg-black/50 flex justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-white">
                        <span className=''>See Details</span>
                    </a>
                </div>
                <div className="p-2 text-center text-black dark:text-white">
                    <h5 className='text-[16px] whitespace-nowrap overflow-hidden text-ellipsis max-w-[140px] mx-auto mb-2' title={title}> <a href={`/details/${type}/${id}`} className='text-gray-900 dark:text-gray-100 hover:text-red-600 duration-200'> {title} </a>  </h5>
